    ...
    <nav aria-label="Books navigation">
        <ul class="pagination">
            <li class="page-item <?php echo $page > 1 ? '' : ' disabled'; ?>">
                <a class="page-link" href="<?php echo 'http://shpp.level3/search?search=' . $_GET['search'] . '&offset=' . $offset . '&page=' . ($page - 1); ?>" aria-disabled="<?php echo $page > 1 ? 'false' : 'true'; ?>">Previous</a>
            </li>
            <?php
            for ($i = 1; $i <= $pagesNumber; $i++) {
                if ($i == $page) {
                    echo '<li class="page-item active" aria-current="page">
                            <a class="page-link" href="http://shpp.level3/search?search=' . $_GET['search'] . '&offset=' . $offset . '&page=' . $i . '">'. $i .' <span class="sr-only">(current)</span></a>
                          </li>';
                } else {
                    echo '<li class="page-item"><a class="page-link" href="http://shpp.level3/search?search=' . $_GET['search'] . '&offset=' . $offset . '&page=' . $i . '">'. $i .'</a></li>';
                }
            }
            ?>
            <li class="page-item <?php echo $page < $pagesNumber ? '' : ' disabled'; ?>">
                <a class="page-link" href="<?php echo 'http://shpp.level3/search?search=' . $_GET['search'] . '&offset=' . $offset . '&page=' . ($page + 1); ?>" aria-disabled="<?php echo $page < $pagesNumber ? 'false' : 'true'; ?>">Next</a>
            </li>
        </ul>
    </nav>
</div>